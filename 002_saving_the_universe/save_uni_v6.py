...
def count_switches(search_eng_list, query_list):
    ...
    # if either queries or search engine lists are empty, don't loop. Just return zero.
    while len(query_list) > 0 and len(search_eng_list) > 0:
...
def main():
    ...
    n_cases, case_list = read_data('A-small-practice.in')    
    output_file_name = 'save_uni_v6_out_small.txt'    
